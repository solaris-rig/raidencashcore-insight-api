...
describe('Currency', function() {
  ...
  var raidencashCentralData = {
    ...
    exchange_rates: {
      raidencash_usd: 9.4858840414,
      btc_usd: 682.93,
      btc_raidencash: 0.01388998
    }
  };

  it.skip('will make live request to raidencash central', function(done) {
    var currency = new CurrencyController({});
    var req = {};
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.raidencash_usd);
        (typeof response.data.raidencash_usd).should.equal('number');
        done();
      }
    };
  ...
  it('will retrieve a fresh value', function(done) {
    var TestCurrencyController = proxyquire('../lib/currency', {
      request: sinon.stub().callsArgWith(1, null, {statusCode: 200}, JSON.stringify(raidencashCentralData))
    });
    var node = {
      log: {
        error: sinon.stub()
    ...
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.raidencash_usd);
        response.data.raidencash_usd.should.equal(9.4858840414);
        done();
      }
    };
    currency.index(req, res);
