...
<div align="center">
  <strong>A MatrixPay blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/solaris-rig/raidencashcore-insight-api">
    <img src="https://img.shields.io/travis/solaris-rig/raidencashcore-insight-api/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/raidencashcore-insight-api">
    <img src="https://img.shields.io/npm/v/raidencashcore-insight-api.svg?style=flat-square" alt="NPM version" />
  </a>
</div>


This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/solaris-rig/insight-raidencash-ui.
...
- [Resources](#resources)
- [License](https://github.com/solaris-rig/raidencashcore-insight-api-dash/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g raidencashcore-node@latest
raidencashcore-node create mynode
cd mynode
raidencashcore-node install insight-api
raidencashcore-node start
```
...
```
With all the configuration options available: https://github.com/solaris-rig/raidencashcore-insight-api/blob/master/lib/ratelimiter.js#L10-17
...
### Block
```
  /raidencashcore-insight-api/block/[:hash]
  /raidencashcore-insight-api/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /raidencashcore-insight-api/block-index/[:height]
  /raidencashcore-insight-api/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /raidencashcore-insight-api/rawblock/[:blockHash]
```
...
```
  /raidencashcore-insight-api/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /raidencashcore-insight-api/tx/[:txid]
  /raidencashcore-insight-api/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /raidencashcore-insight-api/rawtx/[:rawid]
  /raidencashcore-insight-api/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /raidencashcore-insight-api/addr/[:addr][?noTxList=1][&from=&to=]
  /raidencashcore-insight-api/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /raidencashcore-insight-api/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /raidencashcore-insight-api/addr/[:addr]/balance
  /raidencashcore-insight-api/addr/[:addr]/totalReceived
  /raidencashcore-insight-api/addr/[:addr]/totalSent
  /raidencashcore-insight-api/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /raidencashcore-insight-api/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /raidencashcore-insight-api/addrs/[:addrs]/utxo
  /raidencashcore-insight-api/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /raidencashcore-insight-api/addrs/utxo
```
...
```
  /raidencashcore-insight-api/txs/?block=HASH
  /raidencashcore-insight-api/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /raidencashcore-insight-api/txs/?address=ADDR
  /raidencashcore-insight-api/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /raidencashcore-insight-api/addrs/[:addrs]/txs[?from=&to=]
  /raidencashcore-insight-api/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /raidencashcore-insight-api/addrs/txs
```
...
POST method:
```
  /raidencashcore-insight-api/tx/send
```
POST params:
```
...
POST method:
```
  /raidencashcore-insight-api/tx/sendix
```
POST params:
```
  rawtx: "signed transaction as hex string"
...
GET method:
```
  /raidencashcore-insight-api/sporks
```
...
GET method:
```
  /raidencashcore-insight-api/gobject/info
```
...
GET method:
```
  /raidencashcore-insight-api/gobject/count
```
...
GET method:
```
  /raidencashcore-insight-api/gobject/list/proposal (or /raidencashcore-insight-api/gobject/list)
```
...
GET method:
```
  /raidencashcore-insight-api/gobject/list/trigger
```
...
GET method:
```
  /raidencashcore-insight-api/gobject/get/[:hash]
  /raidencashcore-insight-api/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
GET method:
```
  /raidencashcore-insight-api/gobject/check/[:hexData]
  /raidencashcore-insight-api/gobject/check/5b5b2270726f706f736[..]
```
...
GET method:
```
  /raidencashcore-insight-api/gobject/deserialize/[:hexData]
  /raidencashcore-insight-api/gobject/deserialize/5b5b2270726f706f736[..]
```

Sample output:
```
{
  "result":"[[\"proposal\",{\"end_epoch\":1519848619,\"name\":\"ghijklmnopqrstuvwxyz01234567891519097947\",\"payment_address\":\"yik5HAgVAgjH1oZKjcDfvcf22bwBNbSYzB\",\"payment_amount\":10,\"start_epoch\":1519097947,\"type\":1,\"url\":\"https://central.raidencash.app/p/test_proposal_1519097947\"}]]",
  "error":null,
  "id":78637
}
```
...
GET method:
```
  /raidencashcore-insight-api/gobject/votes/current/[:hash]
  /raidencashcore-insight-api/gobject/votes/current/fbda8cdc1f48917f53b7d63fbce81c85d6dedd3d0e476e979926dfd154b84034
```
...
GET method:
```
  /raidencashcore-insight-api/governance/budget/[:blockIndex]
  /raidencashcore-insight-api/governance/budget/79872
```
...
POST method:
```
  /raidencashcore-insight-api/gobject/submit
```
...
### Masternodes List
```
  /raidencashcore-insight-api/masternodes/list
```
### Validate Masternode
```
  /raidencashcore-insight-api/masternodes/validate/[:payee]
  /raidencashcore-insight-api/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /raidencashcore-insight-api/sync
```

### Live Network P2P Data Sync Status
```
  /raidencashcore-insight-api/peer
```

### Status of the Bitcoin Network
```
  /raidencashcore-insight-api/status?q=xxx
```
...
### Utility Methods
```
  /raidencashcore-insight-api/utils/estimatefee[?nbBlocks=2]
```
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in raidencashd.
- The endpoint for `/peer` is no longer relevant connection to raidencashd is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking raidencashd.
...
- (Medium)[How to setup a MatrixPay Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
